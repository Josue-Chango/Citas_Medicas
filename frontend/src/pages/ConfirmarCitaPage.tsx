...
import { confirmarCita } from '../api/client';
...
export default function ConfirmarCitaPage() {
  ...
  const handleConfirmar = async () => {
    ...
    setConfirmando(true);
    try {
      await confirmarCita(citaId, token);
      setResultado('exito');
      setMensaje('Tu cita ha sido confirmada exitosamente.');
    } catch (err: any) {
      setResultado('error');
      setMensaje(err?.response?.data?.error || 'Error al confirmar la cita');
    } finally {
      ...
      <div className="login-wrapper fade-in">
        ...
        <Card className="card-moderno login-card text-center">
          {resultado === 'idle' && !citaId && (
            <Alert variant="danger">Enlace de confirmación inválido</Alert>
          )}
          ...
          {resultado === 'idle' && citaId && (
            <>
              ...
              <p className="mb-3">Presiona el botón para confirmar tu cita.</p>
              <Button variant="primary" className="btn-moderno px-4" onClick={handleConfirmar} disabled={confirmando}>
                {confirmando ? <><Spinner size="sm" className="me-2" />Confirmando...</> : 'Confirmar Cita'}
              </Button>
            </>
          )}
