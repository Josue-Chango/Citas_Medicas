...
export async function enviarSolicitudCancelacion(cita: Appointment, emailPaciente: string): Promise<{ exito: boolean; destino: string; mensaje: string }> {
  const token = generarTokenCancelacion(cita.id);
  const linkCancelacion = `${FRONTEND_URL}/cancelar?id=${cita.id}&token=${token}`;

  const asunto = 'Confirma la cancelación de tu cita - ESPEcitas';
  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 480px; margin: 0 auto;">
      <div style="background: linear-gradient(135deg, #dc3545, #b02a37); padding: 24px; text-align: center; border-radius: 12px 12px 0 0;">
        <h1 style="color: #fff; margin: 0; font-size: 1.4rem;">ESPEcitas</h1>
        <p style="color: rgba(255,255,255,0.85); margin: 4px 0 0;">Sistema de Gesti\u00f3n de Citas M\u00e9dicas</p>
      </div>
      <div style="padding: 24px; border: 1px solid #e0e0e0; border-top: none; border-radius: 0 0 12px 12px;">
  ...
  try {
    ...
    return { exito: true, destino: emailPaciente, mensaje: `Correo de confirmacion de cancelacion enviado a ${emailPaciente}` };
  } catch {
    return { exito: false, destino: emailPaciente, mensaje: 'Error al enviar el correo de confirmacion de cancelacion' };
  }
}
